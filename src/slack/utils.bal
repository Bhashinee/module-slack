...
function handleArchiveResponse(http:Client slackClient, string url) returns @tainted error|() {
    ...
    json ok = check jsonResp.ok;
    if (ok == FALSE) {
        json|error errorRes = jsonResp.'error;
        if (errorRes is json) {
            return error(SLACK_ERROR_CODE, message = errorRes.toString());
        }
    }       
}
...
function inviteUsersToConversation(http:Client slackClient, string channelId, string users) 
    ...
    if (ok == false) {
        json|error errorRes = jsonPayload.'error;
        if (errorRes is json) {
            return error(SLACK_ERROR_CODE, message = errorRes.toString());
        }
    }
